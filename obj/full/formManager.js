...
class ResizeMoveHandle {
    constructor(container, index, mouseDownListener, touchStartlistener) {
        ...
        for (let i = 0; i < ResizeMoveHandle.RESIZEHANDLE_data[index].length >> 1; i++) {
            let x = 1 + ((size - 2) * ResizeMoveHandle.RESIZEHANDLE_data[index][i << 1]) / 12.0;
            let y = 1 + ((size - 2) * ResizeMoveHandle.RESIZEHANDLE_data[index][1 + (i << 1)]) / 12.0;
            d += i == 0 ? 'M ' : 'L ';
...
ResizeMoveHandle.HANDLESIZE = Math.round(20 * (1 + 2 * (YoctoVisualization.constants.guiDPIFactor - 1)));
...
ResizeMoveHandle.RESIZEHANDLE_data = [[0, 0, 0, 7, 2, 5, 7, 10, 10, 7, 5, 2, 7, 0],
    ...
    [6, 0, 8, 2, 7, 2, 7, 5, 10, 5, 10, 4, 12, 6,
...
export class YWidget {
    ...
    get confirmDeleteString() { return "not defined"; }
    //#endif
    snapshot() {
        let w = 1024;
        let h = 1024;
    ...
    get isBeingEdited() { return YWidget.currentEdited == this; }
    static stopEdition() {
    ...
    edit() {
        ...
        if (YWidget.editHandles.length <= 0) {
            ...
            */
            ...
            document.addEventListener("touchend", (e) => { if (YWidget.currentEdited != null)
                YWidget.currentEdited.touchCaptureStop(e); }, { passive: false });
            document.addEventListener("mousemove", (e) => { if (YWidget.currentEdited != null)
                YWidget.currentEdited.mouseCaptureRun(e); }, { passive: false });
            document.addEventListener("mouseup", (e) => { if (YWidget.currentEdited != null)
                YWidget.currentEdited.mouseCaptureStop(e); }, { passive: false });
        }
        this.DrawHandles(this.UIContainer.offsetLeft, this.UIContainer.offsetTop, this.UIContainer.offsetWidth, this.UIContainer.offsetHeight);
    ...
    DrawHandles(offsetLeft, offsetTop, offsetWidth, offsetHeight) {
        ...
        let borderMatrix = YDataRendering.Matrix3x3.newTranslateMatrix(-borderWidth, -borderWidth); // border compensation matrix
    ...
    handleMouseStartCapture(pageX, pageY, handleIndex) {
        ...
        let p = this.resizeCaptureOriginalMatrix.multiplyByV(YDataRendering.Vector3.FromXYCoord(pageX, pageY)).toPoint();
    ...
    handleCaptureRun(pageX, pageY) {
        ...
        let B = YDataRendering.Matrix3x3.newTranslateMatrix(newLeft - this.resizeCaptureOriginalLeft, newTop - this.resizeCaptureOriginalTop);
        let borderWidth = parseFloat(this.UIContainer.style.border);
        let borderMatrix = YDataRendering.Matrix3x3.newTranslateMatrix(-borderWidth, -borderWidth); // border compensation matrix
    ...
    touchCaptureStop(e) { this.mouseCaptureStop(null); }
    mouseCaptureStop(e) {
        if (!this.resizeCaptureRunning)
            return;
        this.resizeCaptureRunning = false;
...
//#ifndef READONLY
YWidget.HANDLESIZE = Math.round(20 * (1 + 2 * (YoctoVisualization.constants.guiDPIFactor - 1))); // TO delete eventually
...
export class gaugeWidget extends YWidget {
    //#ifndef READONLY
    getPropertiesXml() { return this.prop.getXml(1); }
    //#endif
    ...
    contextMenuIsOpening() {
        ...
        //#ifndef READONLY
        if (YoctoVisualization.YWebPage.editMenuItem == null)
            return;
    ...
    PropertyChanged2(src) {
        let info = new YoctoVisualization.PropPathinfo();
    ...
    edit() {
        super.edit();
        YoctoVisualization.YWebPage.EditObject(this, this.prop, (src) => { this.PropertyChanged2(src); }, (src) => { return this.Get_PropertyValue(src); }, () => { this.editStopped(); });
    }
    refreshProperties() {
...
export class graphWidget extends YWidget {
    ...
    contextMenuIsOpening() {
        ...
        for (let i = 0; i < YoctoVisualization.YWebPage.markersSubMenuItems.length; i++) {
    ...
    //#ifndef READONLY
    get confirmDeleteString() { return "Do you really want to delete this graph ?"; }
    //#endif
    constructor(node, editor, x, y, width, height) {
        super(node, editor, x, y, width, height);
        //#ifndef READONLY
        ...
        //#ifndef READONLY
        this._graph.setMarkerCaptureCallbacks((m) => { this.MarkedCaptureStarted(m); }, (m) => { this.MarkedCaptureStopped(m); });
        //#endif
