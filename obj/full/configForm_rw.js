...
export class configForm {
    ...
    static hubStateChanged(source) {
        ...
        for (let i = 1; i < configForm._Hubtable.childElementCount; i++) // skip the header
         {
            if (configForm._Hubtable.childNodes[i]["YHUB"] == source) {
                ...
                switch (source.ConnectionState) {
                    case 2 /* YoctoVisualization.HubState.CONNECTED */:
                        configForm._Hubtable.childNodes[i].childNodes[0].innerHTML = YoctoVisualization.ressources.OkIcon((1.2 * configForm.fontSize).toString(), true, false, false, false, " ");
                        break;
                    case 3 /* YoctoVisualization.HubState.FAILURE */:
                        configForm._Hubtable.childNodes[i].childNodes[0].innerHTML = YoctoVisualization.ressources.FailedIcon((1.2 * configForm.fontSize).toString(), true, false, false, false, " ");
                        break;
                    default:
                        configForm._Hubtable.childNodes[i].childNodes[0].innerHTML = YoctoVisualization.ressources.DontKnowIcon((1.2 * configForm.fontSize).toString(), true, false, false, false, " ");
                        break;
                }
            }
        }
    ...
    static AddHubToUI(hub) {
        ...
        switch (hub.ConnectionState) {
            ...
            case 3 /* YoctoVisualization.HubState.FAILURE */:
                ...
                break;
            case 1 /* YoctoVisualization.HubState.CONNECTING */:
            default:
    ...
    static deleteSelectedHub() {
        ...
        YoctoVisualization.confirm.ask("Do you really want to remove connection " + configForm._selectedHub.toString() + " ?" + ExtraMessage, () => {
            ...
            for (let i = configForm._Hubtable.childElementCount - 1; i > 0; i--) // skip the header
             {
    ...
    static show() {
        if (configForm._window == null) {
            ...
            let params = new YoctoVisualization.newWindowParam(configForm.GUIcoef);
            ...
            new YoctoVisualization.InputFieldManager(configForm.inputResolution, 6 /* YoctoVisualization.InputFieldManager.dataType.DATA_STRICT_POSITIVE_FLOAT */, false, Number.NaN, Number.NaN, (sender, refreshInput) => { configForm.captureDPIChange(configForm.inputResolution); });
            ...
            let sizingOptions = YDataRendering.YDataRenderer.CaptureFormats.Keep.sibblings;
            ...
            new YoctoVisualization.InputFieldManager(configForm.inputCaptureWidth, 3 /* YoctoVisualization.InputFieldManager.dataType.DATA_STRICT_POSITIVE_INT */, false, Number.NaN, Number.NaN, (sender, refreshInput) => { configForm.captureWidthChange(configForm.inputCaptureWidth); });
            ...
            new YoctoVisualization.InputFieldManager(configForm.inputCaptureHeight, 3 /* YoctoVisualization.InputFieldManager.dataType.DATA_STRICT_POSITIVE_INT */, false, Number.NaN, Number.NaN, (sender, refreshInput) => { configForm.captureHeightChange(configForm.inputCaptureHeight); });
            ...
            new YoctoVisualization.InputFieldManager(maxSensorRecs, 2 /* YoctoVisualization.InputFieldManager.dataType.DATA_POSITIVE_INT */, false, Number.NaN, Number.NaN, (sender, refreshInput) => { configForm.maxSensorRecsChange(maxSensorRecs); });
            ...
            new YoctoVisualization.InputFieldManager(maxSeriesRecs, 2 /* YoctoVisualization.InputFieldManager.dataType.DATA_POSITIVE_INT */, false, Number.NaN, Number.NaN, (sender, refreshInput) => { configForm.maxSeriesRecsChange(maxSeriesRecs); });
            ...
            new YoctoVisualization.InputFieldManager(maxDataLogRecs, 1 /* YoctoVisualization.InputFieldManager.dataType.DATA_INT */, false, -1, Number.NaN, (sender, refreshInput) => { configForm.maxDataLogRecsChange(maxDataLogRecs); });
            ...
            new YoctoVisualization.InputFieldManager(DPIoverride, 5 /* YoctoVisualization.InputFieldManager.dataType.DATA_POSITIVE_FLOAT */, false, Number.NaN, Number.NaN, (sender, refreshInput) => { configForm.DPIoverrideChange(checkbox3, DPIoverride); });
    ...
    static dbleClickBringsUpContextMenuChange(source) {
        let prev = YoctoVisualization.constants.dbleClickBringsUpContextMenu;
        YoctoVisualization.constants.dbleClickBringsUpContextMenu = source.checked;
        if (prev != YoctoVisualization.constants.dbleClickBringsUpContextMenu)
            YoctoVisualization.constants.edited = true;
    }
    static verticalDragZoomChange(source) {
        let prev = YDataRendering.YGraph.verticalDragZoomEnabled;
        YDataRendering.YGraph.verticalDragZoomEnabled = source.checked;
        if (prev != YDataRendering.YGraph.verticalDragZoomEnabled)
            YoctoVisualization.constants.edited = true;
    }
    ...
    static captureDPIChange(source) {
        let prev = YoctoVisualization.constants.captureDPI;
        YoctoVisualization.constants.captureDPI = parseInt(source.value);
        if (YoctoVisualization.constants.captureDPI != prev)
            YoctoVisualization.constants.edited = true;
        configForm.refreshSizeParams();
    }
    static captureWidthChange(source) {
        let prev = YoctoVisualization.constants.captureWidth;
        YoctoVisualization.constants.captureWidth = parseInt(source.value);
        if (YoctoVisualization.constants.captureWidth != prev)
            YoctoVisualization.constants.edited = true;
        configForm.refreshSizeParams();
    }
    static captureHeightChange(source) {
        let prev = YoctoVisualization.constants.captureHeight;
        YoctoVisualization.constants.captureHeight = parseInt(source.value);
        if (YoctoVisualization.constants.captureHeight != prev)
            YoctoVisualization.constants.edited = true;
        configForm.refreshSizeParams();
    }
    static captureSizeSelectChange(source) {
        let prev = YoctoVisualization.constants.captureSizePolicy;
        ...
        if (YoctoVisualization.constants.captureSizePolicy != prev)
            YoctoVisualization.constants.edited = true;
        configForm.refreshSizeParams();
    }
    static refreshSizeParams() {
        let value = YDataRendering.YDataRenderer.CaptureFormats.fromString(YDataRendering.YDataRenderer.CaptureFormats, configForm.inputSizeSelect.value);
    ...
    static downloadCurrentConfig() {
        ...
        let data = "data:text/xml;base64," + btoa(YoctoVisualization.YWebPage.get_XmlData());
    ...
    static captureFormatChange(source) {
        ...
        let prev = YoctoVisualization.constants.captureType;
        ...
        if (YoctoVisualization.constants.captureType != prev)
            YoctoVisualization.constants.edited = true;
    }
...
configForm.fontSize = YoctoVisualization.constants.generalFontSize;
//#endif
